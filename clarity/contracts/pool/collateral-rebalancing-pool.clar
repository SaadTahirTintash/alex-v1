                ...
                (dy-weighted 
                    (if (is-eq token-x token-y)
                        ...
                        (if (is-eq token-x .token-wstx)
                            (get dx (try! (contract-call? .fixed-weight-pool swap-y-for-wstx collateral u50000000 dx none)))
                            (if (is-eq token-y .token-wstx)
                                (get dy (try! (contract-call? .fixed-weight-pool swap-wstx-for-y token u50000000 dx none)))
                                (if (is-some (contract-call? .fixed-weight-pool get-pool-exists token collateral u50000000 u50000000))
                                    (get dx (try! (contract-call? .fixed-weight-pool swap-y-for-x token collateral u50000000 u50000000 dx none)))
                                    (get dy (try! (contract-call? .fixed-weight-pool swap-x-for-y collateral token u50000000 u50000000 dx none)))
                                )
                            )
                        )
                    )
